...
\begin{align}
P(Y = 1) &= {n \choose r} p^r(1 - p)^{n - r} \\
&= {4 \choose 1} 0.2^1(1 - 0.8)^{4 - 1} \\
&= 4 \times 0.2 \times 0.2^3 \\
&= 4 \times 0.2 \times 0.2^3 \\
...
r &= 0 \\
P(Y = 0) &= {n \choose r} p^r(1 - p)^{n - r} \\
&= {4 \choose 0} 0.2^0(1 - 0.8)^{4 - 0} \\
&= 1 \times 12\times 0.2^4 \\
...
3. For each of the following cases, indicate whether the use of a normal approximation to the binomial distribution is appropriate.
   ...
   b) $n = 30 $ and $p = 0.1$.  $np =$ `r 30 * 0.1` $\geq 5$ so no, the normal approximation to the binomial distribution is not appropriate.
   c) $n = 30 $ and $p = 0.9$.  $np =$ `r 30 * 0.9` $\geq 5$ but $n(1 - p) =$ `r 30 * (1 - 0.9)` $\geq 5$ so no, the normal approximation to the binomial distribution is not appropriate.
...
\begin{align}
P\left(0 \leq X \leq 1\right) &= P\left(\frac{0 - \mu}{\sigma} \leq \frac{X - \mu}{\sigma} \leq \frac{1 - \mu}{\sigma}\right) \\
&= P\left(\frac{0 - 3}{\sqrt{2}} \leq Z \leq \frac{1 - 3}{\sqrt{2}}\right) \\
&= P\left(\frac{-3}{\sqrt{2}} \leq Z \leq \frac{-2}{\sqrt{2}}\right) \\
&= P\left(`r z1 = -3 / sqrt(2); z1` \leq Z \leq `r z2 <- -2 / sqrt(2); z2`\right) \\
&= P\left(X \leq 1\right) - P\left(X \leq 0\right) \\
&= P\left(Z \leq `r z2`\right) - P\left(Z \leq `r z1`\right) \\
\end{align}
\begin{align}
\text{From the standard normal tables using } Z_1 = -2.12 \text{ and } Z_2 = -1.41
\end{align}
\begin{align}
...
&= `r sprintf("%.3f", pnorm(20.5, xmean, xsd))`
\end{align}
...
&= `r sprintf("%.3f", pnorm(19.5, xmean, xsd))`
\end{align}
