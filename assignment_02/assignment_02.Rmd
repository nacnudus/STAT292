...
Children of different ages are timed doing a manipulative task which requires hand-eye coordination.  The data are:
...
```{r assumptions_tasktime}
ggplot(tasktime, aes(age, time)) + geom_boxplot() + 
stat_summary(fun.y = mean, shape = 1, col = 'red', geom = 'point')

leveneTest(time ~ age, data = tasktime, center = mean)
# note the difference from SAS, which uses the squared residuals (by default)
```

Boxplots showed symmetry in all groups, but not constant variance.  Levene's test of the homogeneity of variance was insignificant at the 5% level.  Possibly, because of the relatively small data set ($n = 16$), it did not have the power to detect the differences that were clear in the boxplots.
...
```{r tasktime_anova}
kable(anova(lm(time ~ age, data = tasktime)))
```
...
```{r assumptions_skinks}
ggplot(skinks, aes(species, temperature)) + geom_boxplot() + 
stat_summary(fun.y = mean, shape = 1, col = 'red', geom = 'point')

leveneTest(temperature ~ species, data = skinks, center = mean)
# note the difference from SAS, which uses the squared residuals (by default)
...
```{r skinks_anova}
kable(anova(lm(temperature ~ species, data = skinks)))
```
